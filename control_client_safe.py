import socket, json, time

HOST='127.0.0.1'
PORT=53421

def send(req, timeout=2.0):
    s = socket.create_connection((HOST, PORT), timeout=timeout)
    payload = json.dumps(req) + '\n'
    s.sendall(payload.encode('utf-8'))
    # read until newline
    data = b''
    while True:
        chunk = s.recv(4096)
        if not chunk:
            break
        data += chunk
        if b'\n' in chunk:
            break
    s.close()
    try:
        text = data.decode('utf-8').strip()
    except:
        text = repr(data)
    print("->", req)
    print("<-", text)
    return text


if __name__ == "__main__":
    # quick test sequence:
    send({"cmd":"list_devices"})
    # FOR DEV: radio input (7,4) loopback (6,1)
    INPUT_ID = 7
    OUTPUT_ID = 4

    send({"cmd":"open", "input": INPUT_ID, "output": OUTPUT_ID, "sr": 48000})
    time.sleep(0.1)

    send({"cmd":"send_frame","mode":"mfsk","speed":"MEDIUM","payload_bits":"1011001110001010"})
    time.sleep(0.5)
    send({"cmd":"send_frame","mode":"mfsk","speed":"FAST","payload_bits":"0101010001000101010100110101010001001001010011100100011100100000010101000100010101010011010101000100100101001110010001110010000001001000010101010101001101001000001000000101000001010010010"})

    # # send an MFSK frame
    # send({
    #  "cmd":"send_frame",
    #  "mode":"mfsk",
    #  "mfsk_m":8,
    #  "mfsk_center":1500.0,
    #  "mfsk_spacing":15.0,
    #  "symbol_len_ms":100,
    #  "preamble_repeats":4,
    #  "payload_bits":""
    # })
    # wait a little so worker can process
    time.sleep(0.5)
    send({"cmd":"status"})
    send({"cmd":"close"})